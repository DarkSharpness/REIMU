...
BranchPredictor::BranchPredictor() : table() {
...
auto BranchPredictor::predict(target_size_t pc) const -> bool {
    auto index = this->get_index(pc);
    constexpr auto kHalf = kMask >> 1;
    return this->get_bits(index) > kHalf;
}

auto BranchPredictor::update(target_size_t pc, bool taken) -> void {
...
auto BranchPredictor::get_index(target_size_t pc) -> target_size_t {
    static_assert(sizeof(command_size_t) == 4);
    return ((pc / sizeof(command_size_t)) & (_Nm - 1)) * kBits;
}

auto BranchPredictor::get_bits(target_size_t index) const -> target_size_t {
    const auto [which, offset] = div_mod <kDigit> (index);
    return (this->table[which] >> offset) & kMask;
}

auto BranchPredictor::set_bits(target_size_t index, target_size_t delta) -> void {
    const auto [which, offset] = div_mod <kDigit> (index);
