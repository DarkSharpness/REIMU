...
public:
    ...
    constexpr _Fn_t get_deleter() const noexcept {
        return this->_M_deleter_fn;
    }
    constexpr auto operator()(_Bp *ptr) const {
        return this->_M_deleter_fn(ptr);
    }
...
public:
    ...
    template<typename _Tp> requires std::derived_from<_Tp, _Bp>
...
template <typename _Bp>
inline bool operator == (const derival_ptr<_Bp> &lhs, std::nullptr_t)
noexcept { return lhs.get() == nullptr; }

template <typename _Bp>
inline bool operator == (std::nullptr_t, const derival_ptr<_Bp> &rhs)
noexcept { return rhs.get() == nullptr; }
