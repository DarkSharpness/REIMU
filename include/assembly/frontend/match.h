...
inline auto match_value_aux(TokenStream stream) -> bool {
...
inline void match_type_aux(TokenStream &stream, std::tuple<_Args...> &ref) {
    constexpr auto __consume = [](TokenStream &stream) {
        throw_if(
            stream.empty() || stream[0].type != Token::Type::Comma,
...
[[nodiscard]]
inline auto match(TokenStream &stream) -> std::tuple<_Args...> {
...
[[nodiscard]]
inline auto match(const TokenStream stream) noexcept -> bool {
